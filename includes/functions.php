...
function can_add_rooms($id, $kwid) {
    $db = new db;
    ...
    //if (isset($_SESSION['user_id']) and $kwid >= $db->get_next_kwds()) {
        $jobs = $db->get_user_job($_SESSION['user_id'], $kwid);
...
function get_event_dates($cdate, $name='date') {
    global $kwds;
    $begin = strtotime($kwds['start_date']);
    $end=strtotime($kwds['end_date']);
...
// Checks to see if the user has a scheduler role
function is_class_scheduler($id, $n) {
    $db = new db;
    // Checks to see if the user is logged in
    if (isset($_SESSION['user_id'])) {
        $result = $db->get_user_job($_SESSION['user_id'], $n);
        // If jobID is 1, 2, 6, 7, 8, or 9 the you can change schedule
        if (count($result) > 0 && ($result[0]['id'] == 1 || $result[0]['id'] == 2 || $result[0]['id'] == 6 ||
                $result[0]['id'] == 7 || $result[0]['id'] == 8 || $result[0]['id'] == 9 || $result[0]['id'] == 16)) {
            return true;
        }
    }
...
function is_proceeding_editor($id, $n) {
    $db = new db;
    // Checks to see if the user is logged in
    if (isset($_SESSION['user_id'])) {
        $result = $db->get_user_job($_SESSION['user_id'], $n);
        // If jobID is 1, 2, 13 the you can change schedule
        if (count($result) > 0 && ($result[0]['id'] == 1 || $result[0]['id'] == 2
                 || $result[0]['id'] == 13 || $result[0]['id'] == 16)) {
            return true;
        }
    }
...
function is_teacher($class, $user) {
    $db = new db;
    $result = $db->is_teacher($class, $user);
    return $result;
}
...
function redirect($location, $kwds = 0) {
    switch ($location) {
        default:
        case 'index':
            $url = 'index.php?kwds='.$kwds;
...
function sanit($input) {
    ...
    $output = mysqli_real_escape_string(mysqli_connect(DB_HOST, DB_USER, DB_PASS, DB_NAME),$input2);
    return $output;
}
